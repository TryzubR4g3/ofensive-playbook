# Hardcoded Credentials in Binaries

Used on: **Overwatch**

Binaries dropped in SMB shares or file servers often ship with their config baked in or shipping an `.exe.config` alongside. Hunting for connection strings and passwords with `strings` + `grep` is almost always worth the five minutes.

## When to Use

- `.exe`, `.dll`, `.pdb`, or `.resources` files are discovered in SMB shares or file servers
- An `.exe.config` file is found alongside a binary
- Running `strings` against a binary reveals `password`, `user`, `sql`, or `connection` keywords

## Key Points

- .NET binaries store string literals as **UTF-16LE**. Default `strings` looks at ASCII only → you miss half the data. Use `-e l`.
- Look at `.exe`, `.dll`, `.exe.config`, `.pdb`, `.resources`.
- Connection strings, URLs, hostnames, JWTs, "admin", "password", "token" are all good grep targets.

## Command Used

```bash
strings -e l overwatch.exe | grep -iE "password|user|sql|connection"
```

- `-e l` — 16-bit little-endian (Windows / .NET).
- `-e L` — 32-bit little-endian.
- `-e b` — 16-bit big-endian.

Result on **Overwatch**:
```
Server=localhost;Database=SecurityLogs;User Id=sqlsvc;Password=TI0LKcfHzZw1Vv;
```

## Expanded Grep

```bash
strings -e l overwatch.exe | grep -iE "password|passwd|pwd|user|admin|token|secret|apikey|connstr|connection|http|https|smtp|ftp"
```

## Dump All String Variants

```bash
for w in s S l L b B; do
  echo "--- width $w ---"
  strings -e $w overwatch.exe
done > all-strings.txt
```

## Follow-ups

- With `.exe.config`: open it directly, often contains Entity Framework connection strings.
- With a .NET exe: decompile with **ILSpy** / **dnSpy** to see secrets that exist only as constants after JIT obfuscation.
- Validate the credential immediately:
  ```bash
  netexec mssql <IP> -u <user> -p '<pass>' --port <port>
  netexec smb <IP> -u <user> -p '<pass>'
  ```


