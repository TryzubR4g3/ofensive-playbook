# Vulnversity Â— TryHackMe Writeup
...
3. [Initial Access Â— `.phtml` Upload Bypass](#3-initial-access--phtml-upload-bypass)
4. [Foothold (`www-data`)](#4-foothold-www-data)
5. [Privilege Escalation Â— Systemd Unit Drop](#5-privilege-escalation--systemd-unit-drop)
...
| 139, 445/tcp | Samba 4 |
| 3128/tcp | Squid 4.10 |
| **3333/tcp** | Apache 2.4.41 Â— *Vuln University* |
...
## 3. Initial Access Â— `.phtml` Upload Bypass
...
# Why here: verificar archivos, permisos o loot en la ruta actual.
ls -l /bin/bash
# -rwsr-xr-x 1 root root Â… /bin/bash
...
`-p` keeps the EUID intact when bash is SUID Â— without it, modern bash drops privileges.
...
- A `/uploads/` directory with **directory listing on** is half the attack Â— visit it after every upload to grab the on-disk path.
...
- `Type=oneshot` + `ExecStart=/bin/bash -c "chmod +s /bin/bash"` is the cleanest unit privesc payload Â— survives reboot, no listener, instant `bash -p`.
