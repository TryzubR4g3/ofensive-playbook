# Chronicle â€” TryHackMe Writeup

**Target:** `TARGET_IP` (10.128.189.151 at time of solve)
**OS:** Linux (Ubuntu)
**Difficulty:** Easy
**Tech stack:** Apache 2.4.29, Werkzeug 1.0.1 (Python 3.6.9), Git
**Exploit chain:** feroxbuster â†’ exposed `.git` in `/old/` â†’ `git show` leaks API key + default creds â†’ ffuf user brute on API â†’ `tommy:DevMakesStuff01` â†’ SSH â†’ `.mozilla` profile exfil from `carlJ` â†’ hashcat mozilla hash â†’ `firefox_decrypt` â†’ `carlJ:Pas$w0RD59247` â†’ SSH as `carlJ` â†’ SUID `smail` binary â†’ buffer overflow â†’ root
...
```
nmap â†’ 22, 80 (Apache), 8081 (Werkzeug)
    â†’
feroxbuster :80 â†’ /old/note.txt, /old/.git
feroxbuster :8081 â†’ /login, /api, /forgot
    â†’
wget --recursive /old/.git â†’ git show â†’ API key 7454c262d0d5a3a0c0b678d6c0dbc7ef
    â†’
ffuf user brute on /api/FUZZ â†’ tommy â†’ DevMakesStuff01
    â†’
ssh tommy@$TARGET â†’ user foothold
    â†’
/home/carlJ/.mozilla â†’ scp to attacker â†’ mozilla2hashcat â†’ hashcat -m 26100 â†’ password1
    â†’
firefox_decrypt â†’ carlJ:Pas$w0RD59247
    â†’
ssh carlJ@$TARGET â†’ ~/mailing/smail (SUID, buffer overflow)
    â†’
pwntools payload â†’ ret2libc (system + /bin/sh) â†’ root
```
...
2. [Web Enumeration â€” Dual Servers](#2-web-enumeration--dual-servers)
3. [Git History Disclosure â€” API Key Recovery](#3-git-history-disclosure--api-key-recovery)
4. [Initial Access â€” API User Brute-Force + SSH](#4-initial-access--api-user-brute-force--ssh)
5. [User Pivot â€” Firefox Credential Extraction](#5-user-pivot--firefox-credential-extraction)
6. [Privilege Escalation â€” SUID Buffer Overflow](#6-privilege-escalation--suid-buffer-overflow)
...
## 2. Web Enumeration â€” Dual Servers
...
Key hits:
- `:80` â†’ `/old/note.txt`, `/old/.git`
- `:8081` â†’ `/login`, `/api`, `/forgot`, `/static/js/forget.js` (API key removed from client-side code)
...
## 3. Git History Disclosure â€” API Key Recovery
...
Tools: [git](../../../tools/devops/git.md).

---

## 4. Initial Access â€” API User Brute-Force + SSH
...
- API endpoints behind removed client-side code are still live â€” always fuzz for undocumented routes and parameters.
...
- SUID binaries that accept user input should always be tested for buffer overflow. `ldd` â†’ libc base â†’ ret2libc is the fastest path when ASLR is off or predictable.
- Browser-stored passwords often reuse the same credential across services â€” always try the recovered password on SSH/SMB/WinRM.
