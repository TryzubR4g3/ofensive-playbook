...
**Nota:** La enumeración de FRRouting y los comandos `mgmt load-config` / `banner motd file` todavía necesitan cierre y explicación antes de extraerlos como técnica definitiva.
...
ES: FRRouting permite exponer archivos locales a través de `banner motd file`. Al apuntar este comando a archivos sensibles y reconectarse, el contenido aparece como el banner MOTD.

EN: FRRouting allows exposing local files via `banner motd file`. Pointing this at sensitive files and reconnecting reveals the content as the MOTD banner.
...
- Network appliance VTY lines with single-password auth are trivially brute-forced — Hydra's `cisco` module handles this natively.
