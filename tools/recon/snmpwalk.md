...
```
Used on: **operationtakeover** - full enumeration of the device using the discovered `pr1v4t3` community string.

### Walk a specific OID subtree
```bash
snmpwalk -v2c -c pr1v4t3 $TARGET .1.3.6.1.4.1.8072.1.3.2
```
Used on: **operationtakeover** - walked the `NET-SNMP-EXTEND-MIB` output table to trigger and read the results of an injected command.
