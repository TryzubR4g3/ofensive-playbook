...
ES: Dentro de SquirrelMail, un email del sistema informaba de un cambio de contraseña SMB. Con la nueva contraseña se accedió al share personal de milesdyson, donde `important.txt` revelaba un directorio web oculto.

EN: Inside SquirrelMail, a system email disclosed a new SMB password. With the new password, milesdyson's personal share was accessible, and `important.txt` revealed a hidden web directory.
...
Full technique: [cuppa-cms-rfi.md](../../exploits/web-rce/cuppa-cms-rfi.md).

ES: El directorio oculto contenía Cuppa CMS, vulnerable a inclusión remota/local de archivos a través del parámetro `urlConfig` en `alertConfigField.php`.

EN: The hidden directory ran Cuppa CMS, vulnerable to remote/local file inclusion via the `urlConfig` parameter in `alertConfigField.php`.
...
ES: Un cronjob de root ejecutaba `tar cf /home/milesdyson/backups/backup.tgz *` dentro de `/var/www/html`. La expansión del wildcard permite inyectar flags de `tar` como nombres de archivo.

EN: A root cron job ran `tar cf ... *` inside `/var/www/html`. The wildcard expansion allows injecting tar flags as filenames.
...
## Related Notes
- [smb-anonymous-enum.md](../../exploits/ad/smb-anonymous-enum.md) — initial enumeration
- [cuppa-cms-rfi.md](../../exploits/web-rce/cuppa-cms-rfi.md) — initial access
- [tar-wildcard-injection.md](../../exploits/privesc-linux/tar-wildcard-injection.md) — privilege escalation
