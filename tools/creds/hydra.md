...
tested one OSINT password against many NTLM-authenticated domain users.
...
found Jenkins credentials through a local SSH port forward.
...
password candidates recovered from SMB logs were replayed against SquirrelMail to recover `milesdyson`.
...
found default-style WebDAV credentials reused for upload access.
...
recovered the FRRouting VTY password that granted access to `configure terminal` and the file-read primitive.
...
brute-forced an SSH login knowing the username from web enumeration.
...
brute-forced SSH for the known `gaara` user.
...
recovered the Asterisk Manager Interface secret for `admin`.
