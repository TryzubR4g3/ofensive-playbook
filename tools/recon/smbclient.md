...
pulled the anonymous SMB note that pointed toward the FTP/NFS path.
...
landed `shell.asp` into a share mirrored by IIS at `:49663/nt4wrksv/`. Pair with [smb-write-iis-execution.md](../../exploits/web-rce/smb-write-iis-execution.md).
...
connected to backup share to read files.
...
authenticated to the writable share used to place the PHP reverse shell in the web path.
