...
**Estado:** WIP / pendiente de documentar escalada completa.
...
ES: Con acceso admin a Joomla, se inyectó una webshell en la plantilla `beez3` mediante el editor de templates.

EN: With Joomla admin access, a webshell was injected into the `beez3` template via the template editor.
...
ES: `yum` es un script Python que carga plugins desde directorios configurados. Se crea un plugin malicioso que ejecuta `/bin/sh` con privilegios de root.

EN: `yum` is a Python script that loads plugins from configured directories. A malicious plugin that runs `/bin/sh` inherits root privileges.
