...
```
Used on: **operationtakeover** - modified the `sysName` OID to verify write access via the `pr1v4t3` community string.
...
```
Used on: **operationtakeover** - exploited write access to define a new extend command that executes arbitrary bash commands as root.
