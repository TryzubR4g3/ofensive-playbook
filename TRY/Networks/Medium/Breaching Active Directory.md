...
ES: Se añadió el dominio del laboratorio al archivo de hosts para resolver el DC y el dominio base.

EN: The lab domain was added to hosts resolution so the DC and base domain resolved correctly.
...
ES: La fase de OSINT proporcionó una contraseña inicial reutilizable contra varios usuarios.

EN: OSINT provided an initial password that could be tested safely against many users.
...
ES: El portal `https://ntlmauth.za.tryhackme.com/` usaba autenticación NTLM. Hydra validó la contraseña OSINT contra una lista de usuarios y encontró varias cuentas válidas.

EN: `https://ntlmauth.za.tryhackme.com/` used NTLM authentication. Hydra tested the OSINT password against a username list and found several valid accounts.
...
ES: El panel de impresora en `https://printer.za.tryhackme.com/settings` no requería credenciales y mostraba el usuario LDAP `svcLDAP`, pero ocultaba la contraseña. Al cambiar el servidor LDAP hacia la máquina atacante y bajar las exigencias SASL en un servidor `slapd` rogue, la prueba de conexión envió credenciales en claro que se capturaron con `tcpdump`.

EN: The printer panel at `https://printer.za.tryhackme.com/settings` required no credentials and exposed the LDAP username `svcLDAP`, but hid the password. Redirecting LDAP to the attacker and lowering SASL requirements on a rogue `slapd` server made the connection test send cleartext credentials, captured with `tcpdump`.
...
ES: Responder en la interfaz `breachad` capturó un challenge NetNTLMv2 para `ZA\svcFileCopy`. Hashcat con modo `5600` crackeó el hash usando `rockyou.txt`.

EN: Responder on the `breachad` interface captured a NetNTLMv2 challenge for `ZA\svcFileCopy`. Hashcat mode `5600` cracked it with `rockyou.txt`.
...
ES: Las grandes organizaciones usan **Microsoft Deployment Toolkit (MDT)** junto con **PXE Boot** para desplegar sistemas operativos automáticamente a través de la red. Cuando un equipo nuevo arranca, solicita la imagen de instalación vía DHCP/TFTP. El problema de seguridad es que estas imágenes pueden contener credenciales de dominio almacenadas en claro en el archivo `bootstrap.ini`, que MDT necesita para autenticarse contra el servidor de despliegue durante la instalación.

EN: Large organisations use **Microsoft Deployment Toolkit (MDT)** with **PXE Boot** to deploy operating systems automatically over the network. When a new machine boots, it requests the install image via DHCP/TFTP. The security issue is that these images can contain domain credentials stored in plaintext inside `bootstrap.ini`, which MDT needs to authenticate against the deployment server during installation.
...
### Paso 1: Obtener el nombre del archivo BCD / Step 1: Get the BCD filename

ES: Accedemos a la URL del servidor PXE para obtener los nombres de los archivos BCD disponibles. Estos archivos contienen la configuración de arranque para diferentes arquitecturas.

EN: We access the PXE server URL to list available BCD files. These files store boot configuration for different hardware architectures.
...
> **Nota:** Los nombres de estos archivos se regeneran cada día. Siempre copia el nombre actual de la web antes de continuar. / **Note:** These filenames regenerate daily. Always copy the current name from the web before proceeding.

---

### Paso 2: Conectarse al host intermedio / Step 2: Connect to the jump host

ES: Dado que necesitamos TFTP para descargar los archivos, usamos el host `THMJMP1` al que tenemos acceso SSH como punto de transferencia.

EN: Since we need TFTP to download files, we use `THMJMP1`, to which we have SSH access, as a transfer point.
...
### Paso 3: Descargar y parsear el archivo BCD / Step 3: Download and parse the BCD file

ES: Usamos TFTP para descargar el archivo BCD desde el servidor MDT. Los archivos BCD siempre están en el directorio `/Tmp/`. Luego usamos **PowerPXE** para extraer la ruta de la imagen WIM.

EN: We use TFTP to download the BCD file from the MDT server. BCD files are always in the `/Tmp/` directory. Then we use **PowerPXE** to extract the WIM image path from it.
...
### Paso 4: Descargar la imagen WIM / Step 4: Download the WIM image

ES: Con la ruta de la imagen identificada, la descargamos completa vía TFTP. Es un archivo grande (~340MB), puede tardar varios minutos.

EN: With the image path identified, we download it in full via TFTP. It's a large file (~340MB) and may take several minutes.
...
### Paso 5: Extraer credenciales del bootstrap.ini / Step 5: Extract credentials from bootstrap.ini

ES: PowerPXE busca dentro de la imagen WIM el archivo `bootstrap.ini`, que MDT usa para autenticarse al servidor de despliegue. Este archivo contiene las credenciales en texto claro.

EN: PowerPXE searches inside the WIM image for `bootstrap.ini`, which MDT uses to authenticate to the deployment server. This file contains credentials in cleartext.
...
ES: **McAfee Enterprise Endpoint Security** (y otras aplicaciones desplegadas centralmente) necesitan autenticarse contra el dominio durante su instalación y ejecución. McAfee almacena estas credenciales en una base de datos SQLite llamada `ma.db`, ubicada en una ruta fija del sistema. El campo de contraseña está cifrado, pero con una clave conocida, lo que permite descifrarlo con una herramienta pública. Esta técnica aplica a cualquier aplicación de gestión centralizada que guarde credenciales localmente en archivos de configuración.

EN: **McAfee Enterprise Endpoint Security** (and other centrally deployed applications) need to authenticate against the domain during installation and operation. McAfee stores these credentials in a SQLite database called `ma.db`, located at a fixed system path. The password field is encrypted, but with a known key, which means it can be decrypted with a public tool. This technique applies to any centralised management application that stores credentials locally in configuration files.
...
### Paso 1: Localizar y copiar la base de datos / Step 1: Locate and copy the database

ES: La base de datos `ma.db` se encuentra siempre en la misma ruta en todos los hosts donde McAfee Agent está instalado.

EN: The `ma.db` database is always at the same path on every host where McAfee Agent is installed.
...
### Paso 2: Abrir la base de datos y localizar las credenciales / Step 2: Open the database and find the credentials

ES: Abrimos la base de datos con `sqlitebrowser` y navegamos a la tabla `AGENT_REPOSITORIES`. Nos interesa la segunda entrada: los campos `DOMAIN`, `AUTH_USER` y `AUTH_PASSWD`.

EN: We open the database with `sqlitebrowser` and navigate to the `AGENT_REPOSITORIES` table. We're interested in the second entry: the `DOMAIN`, `AUTH_USER`, and `AUTH_PASSWD` fields.
...
**Valores encontrados / Values found:**
...
> La contraseña está cifrada con un algoritmo propio de McAfee (no es base64 puro). Se necesita el script específico para descifrarla. / The password is encrypted with McAfee's own algorithm (not plain base64). A specific script is needed to decrypt it.

---

### Paso 3: Descifrar la contraseña / Step 3: Decrypt the password

ES: McAfee cifra el campo `AUTH_PASSWD` con una clave conocida y fija. Usamos el script público `mcafee_sitelist_pwd_decrypt.py` para obtener la contraseña en claro.

EN: McAfee encrypts `AUTH_PASSWD` with a known, fixed key. We use the public script `mcafee_sitelist_pwd_decrypt.py` to recover the plaintext password.
...
| Usuario / User | Contraseña / Password | Método / Method |
|---|---|---|
...
ES:
- Los archivos de configuración de aplicaciones desplegadas centralmente son una fuente excelente de credenciales de dominio.
- PXE Boot puede exponer credenciales de servicio en texto claro si `bootstrap.ini` no está protegido.
- McAfee Agent guarda credenciales cifradas con clave pública conocida, lo que equivale a almacenarlas en texto claro desde el punto de vista del atacante.
- Ante cualquier foothold en un host, seguir una metodología sistemática: enumerar archivos de configuración, bases de datos locales, claves de registro y aplicaciones de gestión centralizada.
