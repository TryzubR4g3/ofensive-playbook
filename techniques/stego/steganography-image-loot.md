...
- Often a passphrase recovered from elsewhere on the target (e.g. `passphrase.txt`, base64 in a comment, credential reuse).
...
# oneforall.jpg: PNG image data  <- mismatched extension
# oneforall.jpg: JPEG image data <- legit JPEG, try steghide straight away
```
...
# Look for Comment, Artist, Description, Software, UserComment
# Sometimes the passphrase or a base64 blob is pasted into a comment field
```
...
cat creds.txt          # SSH creds, users, flags, etc.
```
...
## Fallback / Other Stego Tools

| Tool | Use |
|------|-----|
| `zsteg <png>` | PNG / BMP LSB (no passphrase) |
| `binwalk -e <file>` | Appended archives / files behind an image |
| `strings <file> \| head -200` | Dumb-but-fast readable strings |
| `outguess -k <key> -r <in> <out>` | Older JPEG stego format |
